...
class ViewController: UIViewController {
    ...
    let signalIOClient = SignalClient()
    //    let signalIOClient = SignalIOClient()
    ...
    override func viewDidLoad() {
        ...
        self.signalIOClient.delegate = self
    }
    
    @IBAction func connectDidTap(_ sender: UIButton) {
        self.signalIOClient.connect()
    }
    
    @IBAction func disconnectDidTap(_ sender: UIButton) {
        self.signalIOClient.disconnect()
    }
    
    @IBAction func offerDidTap(_ sender: UIButton) {
        self.webRTCClient.offer { (sdp) in
            self.hasLocalSdp = true
            self.signalIOClient.send(sdp: sdp)
        }
    }
...
extension ViewController: SignalIOClientDelegate {
